...
export default function RegistrarSettingsPage() {
  ...
  const readOnlyFields = useMemo(() => {
    return {
      ...
      email: current?.email || "",
      regNo: current?.regNo || "",
      role: current?.roleLabel || current?.roleKey || "",
    };
  }, [current]);
  ...
  return (
    <>
      ...
      <p className="text-slate-600 mt-2">Manage your registrar profile and preferences.</p>
      ...
      <div className="mt-8 rounded-xl border border-black/[.08] dark:border-white/[.145] bg-white/70 dark:bg-white/5 overflow-hidden">
        ...
        <form className="p-4 grid gap-4 md:grid-cols-2" onSubmit={saveProfile}>
          ...
          <div className="md:col-span-2">
            ...
            {(avatarDataUrl || current?.avatarDataUrl) && (
              <div className="mt-2 w-16 h-16 rounded-full overflow-hidden ring-1 ring-black/10">
                ...
                <img
                  src={avatarDataUrl || current?.avatarDataUrl || ""}
                  alt="Avatar preview"
          ...
          <div className="md:col-span-2 flex items-center justify-end gap-2">
            ...
            <button type="submit" className="dash-btn-primary h-10 px-4 text-sm">
              Save changes
            </button>
          </div>
          {loaded && message && (
